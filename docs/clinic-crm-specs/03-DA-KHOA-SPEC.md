...
#### Dashboard và menu Đa khoa

Dùng primary `#1D4ED8`, sidebar navy, workspace `#F4F7FB`, bảng trắng rõ hàng/cột và status có nhãn. Thiết kế thiên điều phối nhiều khoa: vùng queue/visit lớn, KPI vừa đủ, thông tin định danh và thời gian chờ luôn dễ đọc. Không đưa toàn bộ số liệu một bệnh viện nội trú vào scope phòng khám ngoại trú.

- Receptionist/coordinator: KPI Lượt đến/Đang chờ/Chờ kết quả/Việc cần xử lý; hàng chính là queue theo khoa 8/12 và task 4/12; calendar bác sĩ/cận lâm sàng phía dưới hoặc tab. Có switch “Lượt đến” và “Lượt khám khoa” để hiểu số khác nhau.
- Doctor/technician: panel Các lượt được giao/Kết quả cần review/Chỉ định đến hạn; các cảnh báo critical chỉ khi có quyền, owner và ack đúng version. Manager thấy aggregate TAT/workload, không phải raw kết quả của mọi patient.
- Menu riêng: Lượt đến (Visit), Tiếp nhận & sinh hiệu, Điều phối khoa, Tư vấn chuyên khoa, Chỉ định cận lâm sàng, Mẫu bệnh phẩm, Xét nghiệm, Chẩn đoán hình ảnh, Chuyển khám, Đối chiếu thuốc, Theo dõi.

| Screen | Vùng nội dung và trường chi tiết | Thao tác & phản hồi cần thiết |
|---|---|---|
| G01 Visit | List code/patient/branch/check-in/coordinator/status/số khoa/số order pending/due theo quyền. Detail 20% identity+visit nav, 55% encounters/orders/results, 25% task và checklist close; tabs hẹp thay ba cột | Add encounter theo quyền, mở referral/order/invoice giữ visit context; close preview unresolved items và deferred task owner/due. Encounter completed không tự đóng Visit; badge “Đã kết thúc hẹn đầu” tách status Visit |
| G02 Triage/sinh hiệu | Header patient/visit/time/recorder; numeric fields với unit ngay cạnh, origin/source, priority, destination; summary dữ liệu thiếu | Save record, clinician amend bằng revision; không auto gán mức bệnh từ số. Priority≠routine cần reason, giá trị chưa đo null; keyboard nhập theo thứ tự, không dùng slider cho số đo chính xác |
| G03 Queue theo khoa | Lanes cho từng department, card code/giờ chờ/assignee/room/state, filter routine/priority theo quyền; list/table equivalent cho nhiều khoa | Chuyển khoa mở referral form đích/doctor/room/reason; accept tạo encounter+ticket đúng một lần. Không kéo thẻ đổi patient hoặc khóa chuyên khoa của lượt cũ |
| G04 Tư vấn chuyên khoa | Header visit/encounter/department/doctor; form template Nội/Nhi/TMH/Sản phụ khoa, timeline liên quan, tabs diagnosis/orders/prescriptions/reconciliation | Edit chỉ encounter được giao; cần guardian xác minh với trẻ theo context; sensitive field theo clinical scope. Sign riêng từng khoa; coordinator không ký thay người phụ trách |
| G05 Chỉ định cận lâm sàng | Tabs Lab/Imaging/Functional; table order/patient/visit/service/specimen state/result state/requester/assignee/priority/due | Nhận order đã ký, phân công, in nhãn mẫu demo, mở result. Trạng thái mẫu và result hai cột riêng; filter overdue không xem nhầm completed; không thêm thu phí hai lần khi trả kết quả |
| G06 Mẫu bệnh phẩm | Header barcode dạng text+QR/barcode demo, patient/order/visit; list orders cùng mẫu; timeline collected/received/rejected/processing; detail người/time/type/reason | Collect và nhận mẫu đối chiếu định danh, scan/manual barcode có fallback; reject form reason và tạo recollection ref mới. Barcode sai patient 422 không giữ result nhầm; không đổi mã cũ để tái sử dụng |
| G07 Kết quả xét nghiệm | Table test label/value/unit/reference text/flag, result version, specimen, người nhập/người xác nhận; notes/attachment phía dưới; review panel bên phải | Draft/submit/verify/amend; critical do người có quyền đặt tạo ack task đúng requester/version; ack có action note. Chưa verified/released không tick “Đã trả bệnh nhân”; màu abnormal đi kèm label |
| G08 Hình ảnh/thăm dò | Order/modality/body region/performed time, findings/impression text, file preview PDF/image, verified version/reviewer | Upload file đúng order, draft/verify/amend, release riêng; viewer có download theo quyền, zoom và fallback file lỗi. Không vẽ nút DICOM/PACS hoạt động nếu integration P1 chưa có |
| G09 Chuyển khám | List referral code/source department/target/type/priority/owner/status/due?; detail summary đã chọn chia sẻ và timeline | Internal accept → encounter mới cùng visit; decline/cancel cần reason. External document_ready/handed_over phải ghi người nhận/evidence, không tự gửi email bên ngoài. Handed_over không có badge “Đã khám” |
| G10 Đối chiếu thuốc | Theo visit, bảng nguồn đơn/tự khai, item/route/dose text/doctor, decision và reason; highlight duplicate item cần review, phần summary còn pending | Doctor chọn continue/stop/change/review_pending; change dùng đơn mới và void hợp lệ, không inline edit đơn signed. Sign reconciliation cần xử lý pending hoặc documented defer; có timeline người review/version |
| G11 Theo dõi | List patient/visit/problem title/owner/review date/task count/status; detail mục tiêu, tasks và lịch tái khám, revision/release | Tạo/sign/activate/pause/complete theo quyền; task có owner/due, completed task không tự kết luận khỏi bệnh. Lịch mới tạo visit mới khi đến, không nhét vào encounter cũ |

G12 là patient `/mini/care`; admin chỉ release nội dung đã ký hoặc preview projection có quyền, không mở mọi bản nháp dưới nhãn “Xem như bệnh nhân”. Guard release/result version và guardian vẫn áp dụng.

**Đường đi quản trị đa khoa:** C08/G01 check-in → G02 sinh hiệu → G03/G04 khám → G05/G06/G07 hoặc G08 thực hiện → G09 chuyển khoa nếu có → G10 review → G01 close hoặc deferred follow-up → C19/C20 → G11/release. Breadcrumb luôn hiện visit code + encounter khoa hiện tại; quay từ result về đúng order, không nhảy sang encounter đầu của patient khác.

**Bảng điều khiển chuyên khoa:** matrix khoa×queue/status, TAT theo loại, critical ack pending theo version, visit unresolved và task sau khám; không cộng số encounter vào “lượt đến”. Report theo khoa phân bổ thực thu đúng công thức mục 9.7; người dùng có thể click số để xem nguồn charge, không có biểu đồ tiền cộng duplicated joins.

**Cấu hình riêng C24–C26:** department/room/doctor capability, service-result schemas, specimen type và compatibility, form từng khoa, người xác nhận/điều phối, operational due thresholds. C31–C35 ưu tiên tìm khoa, lịch và cơ sở, các thông tin chuẩn bị do chuyên môn duyệt; CMS không đặt rule tự phân khoa từ triệu chứng hoặc thay kết quả khám.
